<div>
    <x-slot name="header">
        <div class="flex justify-between items-center" x-data>
            ...
            <x-button type="button" x-on:click="$dispatch('open-create-keynote-modal')"
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text" id="search"
                            ...
                            placeholder="{{ __('Search key, title or content...') }}">
                    </div>
                    <div>
                        <label for="dateFrom"
                            class="block text-sm font-medium text-gray-700">{{ __('Date From') }}</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($notes as $note)
                    <div wire:key="keynote-{{ $note->id }}"
                        class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200 flex flex-col hover:shadow-md transition-shadow duration-200 relative">
                        <!-- Key Badge -->
                        <div
                            class="absolute top-0 right-0 bg-emerald-100 text-emerald-800 text-xs font-bold px-3 py-1 rounded-bl-lg rounded-tr-lg border-b border-l border-emerald-200">
                        ...
                        <div
                            class="bg-gray-50 px-6 py-3 border-t border-gray-100 flex justify-between items-center text-right">
                            <span class="text-xs text-gray-400">ID: {{ $note->id }}</span>
                            <div class="space-x-3 flex items-center">
                                <button type="button"
                                    x-on:click="$dispatch('open-show-key-note-modal', { keyNote: {{ $note->id }} })"
                                ...
                                <button type="button"
                                    x-on:click="$dispatch('open-edit-keynote-modal', { note: {{ $note->id }} })"
                                ...
                                <button type="button"
                                    x-on:click="$dispatch('open-delete-keynote-modal', { note: {{ $note->id }} })"
                                    class="text-red-600 hover:text-red-900 transition-colors" title="{{ __('Delete') }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="size-5">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                @empty
                    <div
                        class="col-span-1 md:col-span-2 lg:col-span-3 bg-white overflow-hidden shadow-sm sm:rounded-lg p-12 text-center">
